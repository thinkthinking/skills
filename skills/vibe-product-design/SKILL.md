...
**核心节奏是「先写后画再回填」**：先把业务逻辑想清楚、写成初稿文字，再据此画原型图，图出来后再回头
用图校正、补全初稿——图和文字互相校验，不是图文各画各的。全程遇到查不到、推不出的不确定点，都照
`references/澄清问卷.md` 的方法用 `AskUserQuestion` 问用户，不要凌空猜测业务规则或交互细节。
...
6. **依据原型图回填 PRD**：图出来后不是直接贴图收工，而是回头做两件事：
   1. 把 4.2 节每张图的图注（这是什么/关键元素/关键设计决策/交互说明）写完整，用相对路径
      `assets/<文件名>.png` 把图链接进 PRD；
   2. **检查图有没有暴露初稿没想到的东西**——画图时发现的新状态、新边界、漏掉的交互分支，回填到
      对应章节（4.1 的流程说明、5.2 业务规则、5.4 异常处理、第 6 章验收标准、第 7 章测试关注点），
      不是只改第 4 章。这一步是"先写后画再回填"的核心价值：图完成后 PRD 要跟着变得更完整、更准确，
      而不是图文各自为战。回填过程中如果图暴露出新的不确定点，同样先问用户，不要自己猜。

   这两轮写作是**同一次产出内部的节奏**，完成后只算一次交付、只在变更记录里留一条记录（新建 v1.0 /
   修改版本 +1），不需要在版本历史里体现"草稿版"。
7. **格式规范化**：
...
- 不要在模式 B 写初稿、定原型图清单、或依图回填的任何一个环节里凌空猜测业务规则或交互细节——查不到、
