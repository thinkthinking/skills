...
description: >-
  ...
  （基于已有产品，只为一个功能点写清楚设计/逻辑/边界，配低保真原型图，适合日常迭代），并用问卷向
...
理解与分诊   读懂需求本质 → 查产出目录现状 → 问卷确认走哪条路
                ...
                │
                └─ 模式 B · 功能模块 PRD   基于已有产品的单个功能点，问卷澄清 → 配原型图 → 单文档产出
```
...
所有产出都放在同一个根目录下（模式 A 和模式 B 共用，模式 B 的功能文件夹平级放在 `features/` 下）：
...
3. **用 `AskUserQuestion` 问一次路由问题**，确认走哪条路——除非用户的原始表述已经**明确**说了要
   ...
   - **选项 A（模式 B · 功能模块 PRD，多数场景推荐）：** 基于已有产品，只为这一个功能点写清楚
     设计、逻辑、边界，配低保真原型图，写得快、够聚焦，适合日常迭代/新增功能。
...
└── <功能名 slug>/
    ├── PRD.md          # 单文档交付物
    └── assets/         # 低保真原型图，PRD.md 用相对路径 assets/xxx.png 引用
```
...
3. **生成低保真原型图**：按 `references/功能模块PRD.md` 的「原型图生成规范」一节执行——检查/安装
   `zenmux-image-generation`、一次性列出本功能要画的所有界面/状态及其 prompt 供用户确认、生成时
   显式指定输出目录到本功能的 `assets/` 子目录。**不要用该 skill 自己的默认输出路径**，也不要用它
   默认的 4 变体（每个界面 1 张，拿不准给 2 张对比即可）。
